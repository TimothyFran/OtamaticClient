...
void OtamaticClient::onEvent(void (*callback)(OtamaticClientEvent)) {
    _onEvent = callback;
}
...
bool OtamaticClient::applyUpdate() {
    if (! _checkData.isValid()) {
        transmitEvent(OtamaticClientEvent::UpdateFailed);
        return false;
    }
    ...
    while (written < _checkData.size && ! failed) {
        ...
        if (progress != lastProgress) {
            ...
            transmitEvent(OtamaticClientEvent::UpdateProgress);
        }
    }
